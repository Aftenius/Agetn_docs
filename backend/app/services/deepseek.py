...
6) Перед таблицей реквизитов обязательно включи блок спецификации: заголовок <h2>Приложение №1. Спецификация</h2>,
   затем таблицу <table> с колонками: №, Наименование, Количество, Цена за ед., Сумма; минимум 2 строки данных с плейсхолдерами [уточнить: …].
7) Таблица реквизитов сторон — ТОЛЬКО ПОСЛЕ приложения №1, в конце текста, ПЕРЕД таблицей подписей:
   две колонки: слева Поставщик, справа Покупатель (для договора поставки).
...
8) Самый низ — таблица подписей сторон (две колонки).
9) Ставка НДС: бери процент ТОЛЬКО из контекста (структурированные данные, настройки организации). Соответствующие формулировки «НДС N%», «в том числе НДС …». Либо «без НДС» с основанием. Не меняй N на другой процент из головы.
10) В разделе о цене и оплате этапам графика НЕ используй маркированные списки <ul>/<li> и не оформляй этапы через тире в начале строки.
   Каждый этап — отдельный абзац <p> с жёсткой нумерацией вида <strong>2.3.1.</strong>, <strong>2.3.2.</strong> и т.д. (номер подпункта согласуй с номером раздела о расчётах).
...
def generate_contract_structure(
    *,
    ...
    if structured_context.strip():
        struct_part = (
            "\nСтруктурированные данные от пользователя (ОБЯЗАТЕЛЬНО учти в тексте договора, суммы и НДС не выдумывай):\n---\n"
            f"{structured_context.strip()[:14_000]}\n---\n"
        )
...
в тексте плейсхолдеры [уточнить: …]. Списки <ul>/<li> — только где это не противоречит правилу про график оплаты (см. выше).
